...
`--dspark-confidence 0` forces fixed five-draft blocks and is intended for
...
`--ssd-streaming-cache-experts 4000` is different: it normally means exactly
4000 dynamic expert slots, with no routed-prefill reserve. Non-routed weights,
KV cache, graph scratch, and activations need additional memory. Metal
SSD+DSpark has an experimental safety policy for A/B testing: set
`DS4_METAL_DSPARK_SAFE_EXPERT_COUNT=1` to convert a numeric count to bytes and
cap it, when measurable, after accounting for the target's non-routed weights,
the context/KV estimate, and a 2 GiB active reserve for the mmap-backed support
model. It does not yet price the complete batch-prefill workspace or a separate
routed-prefill transient reserve. Startup reports requested/effective slots and
the support reserve. If no safe room can be measured, the explicit count is
retained with a warning rather than making an existing command fail. The policy
is opt-in until real-hardware benchmarks justify changing the CLI default; it
does not affect `NGB` budgets or CUDA/ROCm. Prefer an `NGB` budget for normal
use. The automatic cache budget takes
