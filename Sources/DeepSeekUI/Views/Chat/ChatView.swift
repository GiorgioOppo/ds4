...
struct ChatView: View {
    ...
    private func compactionBanner(c: Conversation,
        ...
        let alreadyCompacted = c.messages.contains {
            $0.role == .system
            && $0.content.hasPrefix(ChatCompactionConstants.marker)
        }
...
/// record stays the job of `PrefillTraceDisclosure`. Kept in this
/// file (rather than its own) so the XcodeGen-generated project
/// picks it up without a regen — the same reason `PrefillIndicator`
/// and `ThroughputBar` live here.
private struct PrefillTokenStreamView: View {
