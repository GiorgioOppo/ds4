...
/// region is allocated.
///
/// Note: this class is public scaffolding; the production inference path
/// holds its KV state directly on `MLA.kvCache`, `Indexer.kvCache`, and
/// `Compressor.kvState`/`scoreState`. To release runtime memory between
/// prompts use `Transformer.releaseCache()`.
