...
// BF16 weight → F32 output. Same layout, single element cast.
kernel void embed_lookup_bf16_to_f32(
    device const bfloat* weight [[buffer(0)]],   // [vocab, D]
    device const int*    ids    [[buffer(1)]],   // [N]
    device float*        out_   [[buffer(2)]],   // [N, D]
    constant uint2&      dims   [[buffer(3)]],   // (N, D)
    uint2 gid [[thread_position_in_grid]]
    ...
    out_[n * D + d] = float(weight[(uint)id * D + d]);
}
