...
| `DS4_DSPARK_GGUF` | percorso o `=1` | non impostata | Apre il supporto DSpark abbinato al checkpoint e abilita l'intero percorso speculativo greedy: tre transformer no-copy, KV privato, proposta Markov/confidence e verifica target esatta. Richiede temperatura 0 e repetition penalty 1. |
| `DS4_DSPARK_DRAFT_TOKENS` | `1...16` | `5` | Limita quanti candidati del blocco DSpark ufficiale da cinque righe vengono inviati al verifier target esatto. Non cambia la forma d'ingresso del transformer di supporto. |
| `DS4_DSPARK_CONFIDENCE` | `0...1` | `0.7` | Soglia confidence per mantenere il prefisso della proposta DSpark. `0` è la modalità diagnostica upstream che forza un blocco fisso; ogni token emesso resta deciso dalla verifica target. |
| `DS4_DSPARK_SCHEDULER` | `0` disabilita | on | Politica adattiva output-preserving portata dall'upstream: salta le code sotto 10 token, applica backoff dopo proposte vuote/a bassa confidence e sospende le finestre poco redditizie mentre i forward target ordinari mantengono aggiornata la KV privata DSpark. Accetta anche i nomi di tuning upstream (`..._WINDOW`, `..._SKIP`, `..._SLOW_SKIP`, `..._NO_DRAFT_SKIP`, `..._COLD_LOW_CONFIDENCE_*`, `..._TAIL_MIN_TOKENS`). |
| `DS4_DSPARK_EXACT_REPLAY` | `=1` | off | Rigioca anche le proposte interamente accettate col decode ordinario per diagnosi A/B bit-level. Disabilita intenzionalmente il commit veloce delle accettazioni complete. |

`DS4Demo inspect-dspark <support.gguf>` stampa metadati, forme e quantizzazioni
senza avviare Metal; `--bind` verifica anche il binding no-copy dei tre stadi.
...
| `DS4_KV_INITIAL` | numero di token | `512` | Frontiera fisica iniziale delle cache NSA di attenzione e indexer. Il tetto logico resta `DS4_DEMO_CONTEXT`, ma i buffer Metal contigui crescono geometricamente solo quando il KV vivo supera questa frontiera. Evita che una sessione vuota da 1M token leghi circa 13 GB di risorse virtuali del compressore. La crescita conserva le righe emesse e non cambia la numerica. |
...
| `DS4_EXPERT_CACHE_HOT_EVICTION` | `=1` | off | A/B lossless: invece della LRU pura, espelle prima l'esperto col rango storico peggiore e usa la recenza come spareggio. Protegge l'hot set dalle scansioni occasionali; può peggiorare quando il workload cambia, quindi resta opt-in finché base/hot-eviction non sono misurati sullo stesso usage profile. |
...
| `DS4_PREAD_SPLIT` | `1...8` | `3` | Con `DS4_EXPERT_PREAD=1`: numero di pread CONCORRENTI per slab di esperto nel riempimento della slot cache. I miss in decode sono pochi per layer (~2-3 × 3 slab ⇒ queue depth NVMe ~6-9), ma il disco raggiunge il suo tetto solo con ~24 richieste in volo: dividere ogni slab in intervalli disgiunti allineati letti in parallelo alza la queue depth a parità di byte. Stessi byte, stessa numerica. |
| `DS4_PREFETCH` | `0/1` | off | Usa una coda CPU e `madvise` per anticipare i pesi non instradati. A/B M1 Pro: 2,33→2,21 tok/s wall e gather 4,22→3,74 GB/s, quindi resta opt-in. |
...
| `DS4_DENSE_Q4` | `=1` (richiede `DS4_DENSE_STREAM=1`) | off | Mantiene residenti le tre proiezioni giganti dell'attention (`q_b`, `output_a`, `output_b`). Su un GGUF **AProjQ4** carica direttamente i byte Q4_K nativi e non crea alcun file `.q4dense`. Su un modello AProjQ8 esegue una sola riquantizzazione Q8_0→Q4_K (**lossy**) e la salva in `<gguf>.q4dense`. Le proiezioni escono dallo stream SSD per-token e il kernel Q4 viene scelto dal tipo reale del tensore GGUF. |
...
| `DS4_EXPERT_ASYNC_SPLIT` | `=0` disabilita | on | Nei layer misti Flash IQ2_XXS/Q2_K della slot-cache, sovrappone la lettura degli esperti mancanti a un pass Metal esatto sui gate/up residenti, quindi fa il join prima della coppia differita e del down-tail. I pesi residenti sono copiati in ~25 MiB di scratch immutabile; una lease GPU blocca scritture look-ahead ritardate nel pool sorgente; il kernel indiretto fissa `NSG=2` come il C. A/B finale da 24 token: 3,77→4,06 tok/s (+7,7%), 3.232.000/3.232.000 logit identici bit per bit; A/B inverso da 12 token: esatto, +6,6%. `=0` ripristina l'acquire sincrono. |
...
|---|---|---|
| `expert gather` domina e la banda effettiva di gather è molto sotto il tetto SSD | Confrontare il default con `DS4_WILLNEED_EXPERTS=0`, poi regolare `DS4_PREAD_SPLIT` intorno al default `3` | Separa read-ahead, churn della page cache e concorrenza delle letture dirette dal GGUF. |
...
Il percorso diretto dal GGUF usa `DS4_PREAD_SPLIT=3` come default.
