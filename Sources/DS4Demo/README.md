...
| `DS4_DENSE_Q4` | `=1` (requires `DS4_DENSE_STREAM=1`) | off | **LOSSY.** Requantizes the two giant plain-matvec projections (`q_b`, `output_b` — Q8_0, 71 of ~145 MB/layer) to Q4_K at load and keeps them RESIDENT (~0.9 GB, locked with `DS4_MLOCK`): half their bytes, RAM-speed reads, and ~3 GB/token removed from the SSD stream — the biggest byte-reduction available once the decode is disk-bound. Uses the already-validated Q4_K matvec kernels. Quality: comparable to a Q4_K conversion of those tensors; verify with the smoke test (`argmax`/logit of the first forward) and by reading a few generations. Load adds a one-time requant pass (~10-30 s). |
