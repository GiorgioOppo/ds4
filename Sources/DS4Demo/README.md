...
| `DS4_EXPERT_PREAD` | `=1` | off | Gli slab degli esperti vengono letti con **`pread` + `F_NOCACHE`** direttamente nei buffer di destinazione invece che con memcpy dall'mmap: il churn (~1 GB/token) **non passa più dalla page cache**, quindi smette di evictare i pesi densi — su 16 GB è la cura quando il profilo mostra `route/attn`/`embed`/`head` dominanti (i densi ri-faultati dall'SSD ogni token). Stessi byte, stesse numeriche; disattiva automaticamente gli hint madvise (inutili senza page cache). Da misurare A/B per macchina. |
...
| `DS4_Q8_NSG` | `1…8` | `4` | Simdgroup-per-threadgroup del matvec **Q8_0 denso** (proiezioni `q_a/q_b/kv/output/router/shared` + low-rank attn-output). È solo **partizione del lavoro**: il kernel divide la riduzione (K) su `NSG` simdgroup e somma esattamente su quelli, shmem e grid sono indipendenti da `NSG`, quindi **qualunque valore dà risultati identici** — cambia solo l'occupancy / il latency-hiding del DRAM. Il default `4` è esattamente la config di riferimento del C (`ds4_gpu_make_q8_0_mv_dispatch`). L'ottimo è **hardware-specifico**: sweepa `2/4/6/8` sul tuo Mac per trovarlo (non possiamo benchmarkare noi). |
