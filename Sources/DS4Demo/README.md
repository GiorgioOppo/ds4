...
| `DS4_DSPARK_DRAFT_TOKENS` | `1...16` | `5` | Caps how many candidates from the official five-row DSpark block are sent to the exact target verifier. It does not change the support-transformer input shape. |
| `DS4_DSPARK_CONFIDENCE` | `0...1` | `0.7` | Confidence threshold for retaining the DSpark proposal prefix. `0` is the upstream diagnostic mode that forces a fixed proposal block; target verification still defines every emitted token. |
| `DS4_DSPARK_SCHEDULER` | `0` disables | on | Output-preserving adaptive admission policy ported from upstream: skips tails shorter than 10 tokens, backs off after empty/low-confidence proposals and pauses low-yield windows while ordinary target forwards keep the private DSpark KV current. The upstream tuning names (`..._WINDOW`, `..._SKIP`, `..._SLOW_SKIP`, `..._NO_DRAFT_SKIP`, `..._COLD_LOW_CONFIDENCE_*`, `..._TAIL_MIN_TOKENS`) are also accepted. |
...
| `DS4_DENSE_Q4` | `=1` (requires `DS4_DENSE_STREAM=1`) | off | Keeps the three giant attention projections (`q_b`, `output_a`, `output_b`) resident. On an **AProjQ4** GGUF their native Q4_K bytes are loaded directly and no `.q4dense` file is created. On an AProjQ8 model they are requantized Q8_0→Q4_K once (**lossy**) and cached in `<gguf>.q4dense`. This removes the projections from the per-token SSD stream and selects the Q4 kernel from the actual GGUF tensor type. |
...
| `DS4_EXPERT_ASYNC_SPLIT` | `=0` disables | on | For mixed Flash IQ2_XXS/Q2_K slot-cache layers, overlaps missing-expert reads with an exact resident gate/up Metal pass, then joins both before the deferred pair/down tail. Resident weights are copied to ~25 MiB immutable scratch; a GPU lease blocks delayed look-ahead writes to the source pool; the indirect kernel fixes `NSG=2` as in C. Final 24-token A/B: 3.77→4.06 tok/s (+7.7%), 3,232,000/3,232,000 logits bit-identical; reversed 12-token A/B: exact, +6.6%. `=0` restores synchronous acquire. |
