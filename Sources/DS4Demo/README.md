...
| `DS4_PREFILL_MM` | `=1` | off (OPT-IN) | Routed FFN of the batched prefill through the `mul_mm_id` matrix-matrix kernels: expert weights are read once per 64×32 tile for ALL the group's tokens instead of once per token (matvec). Same math, different accumulation order (simdgroup MMA, f16 mid) — outputs are close but NOT bit-identical to the matvec path, hence opt-in until A/B-validated. iq2_xxs gate/up + q2_K down only (the Flash shape); groups with `DS4_ACTIVE_EXPERTS` < 6 or fewer than 8 tokens fall back to matvec. Costs ~70 MB of extra transient staging per chunk. |
