...
public final class DeepSeekV4Tokenizer: TokenizerProtocol {
    ...
    /// Same longest-first index without ids, used to neutralize literal control
    /// tokens in untrusted payloads *before* trusted chat framing is rendered.
    private let specialLiteralBytesByFirstByte: [[[UInt8]]]
    ...
    public init(model: GGUFModel) throws {
        ...
        self.specialLiteralBytesByFirstByte = byFirst.map { $0.map(\.bytes) }
    }
    ...
    // MARK: Prompt-boundary containment

    /// Neutralize literal model control tokens inside an UNTRUSTED payload.
    ///
    /// Call this on system/user/history/tool/schema text before handing those
    /// fields to `ChatRenderer`; never call it on the completed rendered prompt,
    /// because the renderer's own BOS/role/DSML framing must remain atomic. A
    /// multi-scalar control token is broken with invisible WORD JOINERs between
    /// its scalars, preserving its readable form while preventing the
    /// special-token scanner from recognizing it. A one-scalar control token is
    /// rendered visibly as `\\u{HEX}` because it cannot be split safely.
    public func neutralizeSpecialTokenLiterals(in text: String) -> String {
        Self.neutralizeSpecialTokenLiterals(in: text, buckets: specialLiteralBytesByFirstByte)
    }

    /// Pure/testable variant of `neutralizeSpecialTokenLiterals(in:)` for a
    /// caller-provided special-token set. Longest match wins and empty/duplicate
    /// entries are ignored.
    public static func neutralizeSpecialTokenLiterals(in text: String,
                                                      specialTokens: [String]) -> String {
        neutralizeSpecialTokenLiterals(in: text, buckets: makeSpecialBuckets(specialTokens))
    }

    /// JSON-aware form for tool schemas and historical call arguments. It first
    /// decodes JSON strings/keys, so a structural literal written as `\\uXXXX`
    /// cannot reappear only after `ChatRenderer` parses the JSON. Invalid JSON is
    /// treated as ordinary untrusted text.
    public func neutralizeSpecialTokenLiterals(inJSON json: String) -> String {
        Self.neutralizeSpecialTokenLiterals(inJSON: json) {
            self.neutralizeSpecialTokenLiterals(in: $0)
        }
    }

    /// Pure/testable JSON-aware variant for a caller-provided special-token set.
    public static func neutralizeSpecialTokenLiterals(inJSON json: String,
                                                      specialTokens: [String]) -> String {
        let buckets = makeSpecialBuckets(specialTokens)
        return neutralizeSpecialTokenLiterals(inJSON: json) {
            neutralizeSpecialTokenLiterals(in: $0, buckets: buckets)
        }
    }

    private static func makeSpecialBuckets(_ specialTokens: [String]) -> [[[UInt8]]] {
        var unique = Set<[UInt8]>()
        var buckets = [[[UInt8]]](repeating: [], count: 256)
        for token in specialTokens {
            let bytes = Array(token.utf8)
            guard !bytes.isEmpty, unique.insert(bytes).inserted else { continue }
            buckets[Int(bytes[0])].append(bytes)
        }
        for i in buckets.indices { buckets[i].sort { $0.count > $1.count } }
        return buckets
    }

    private static func neutralizeSpecialTokenLiterals(inJSON json: String,
        ...
        func visit(_ value: Any) -> Any {
            ...
            if let object = value as? [String: Any] {
                var safe: [String: Any] = [:]
                safe.reserveCapacity(object.count)
                for (key, item) in object { safe[transform(key)] = visit(item) }
                return safe
            }
            return value
        }
        let safeRoot = visit(root)
        guard JSONSerialization.isValidJSONObject(safeRoot),
              let safeData = try? JSONSerialization.data(withJSONObject: safeRoot,
                                                          options: [.sortedKeys, .withoutEscapingSlashes]),
              let safeJSON = String(data: safeData, encoding: .utf8) else {
            return transform(json)
        }
        return safeJSON
    }

    private static func neutralizeSpecialTokenLiterals(in text: String,
                                                        buckets: [[[UInt8]]]) -> String {
        let input = Array(text.utf8)
        guard !input.isEmpty else { return text }
        // The normal case contains no injected control literal. Allocate an
        // output buffer only after the first match, then copy the prefix once.
        var output: [UInt8]?
        var p = 0
        while p < input.count {
            var matched: [UInt8]?
            for candidate in buckets[Int(input[p])] {
                let end = p + candidate.count
                if end <= input.count && input[p..<end].elementsEqual(candidate) {
                    matched = candidate
                    break
                }
            }
            if let matched {
                if output == nil {
                    output = Array(input[..<p])
                    output!.reserveCapacity(input.count + matched.count)
                }
                output!.append(contentsOf: neutralizedLiteral(matched))
                p += matched.count
            } else {
    ...
    private static func neutralizedLiteral(_ bytes: [UInt8]) -> [UInt8] {
        ...
        guard scalars.count > 1 else {
            ...
            return Array("\\u{\(String(scalar.value, radix: 16, uppercase: true))}".utf8)
        }
        var escaped = String.UnicodeScalarView()
        let joiner = "\u{2060}".unicodeScalars.first! // visually inert, breaks exact literal matching.
        for (index, scalar) in scalars.enumerated() {
            if index > 0 { escaped.append(joiner) }
            escaped.append(scalar)
        }
        return Array(String(escaped).utf8)
    }
    ...
    private func isLiteralSpecial(_ s: [UInt8]) -> Bool {
        let bar = DeepSeekV4Tokenizer.fullwidthBar
...
/// Source-compatible alias for existing DS4Engine, DS4Metal and application
/// consumers. New architecture-aware code should prefer the explicit type or
/// store an existential `any TokenizerProtocol`.
public typealias Tokenizer = DeepSeekV4Tokenizer
