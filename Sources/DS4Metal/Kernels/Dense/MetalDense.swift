...
extension MetalRuntime {
    ...
    /// Two Q8_0 matvecs sharing one activation stream and one GPU dispatch.
    public func matmulQ8_0Pair(weightA: [UInt8], weightB: [UInt8], activation: [Float],
                               inDim: Int, outDim: Int) throws -> (a: [Float], b: [Float]) {
        precondition(inDim % 32 == 0)
        let rowBytes = (inDim / 32) * 34
        precondition(weightA.count >= rowBytes * outDim && weightB.count >= rowBytes * outDim)
        precondition(activation.count >= inDim)
        let nsg: Int16 = 4, nr0 = 2
        let args = Self.mulMVArgs(ne00: inDim, ne01: outDim, nb00: 34,
                                  nb01: UInt64(rowBytes), nb02: UInt64(rowBytes * outDim),
                                  ne10: inDim, ne11: 1, nb10: 4,
                                  nb11: UInt64(inDim * 4), ne0: outDim, ne1: 1,
                                  nr0: Int32(nr0))
        guard let wa = device.makeBuffer(bytes: weightA, length: rowBytes * outDim, options: .storageModeShared),
              let wb = device.makeBuffer(bytes: weightB, length: rowBytes * outDim, options: .storageModeShared),
              let xb = device.makeBuffer(bytes: activation, length: inDim * 4, options: .storageModeShared),
              let oa = device.makeBuffer(length: outDim * 4, options: .storageModeShared),
              let ob = device.makeBuffer(length: outDim * 4, options: .storageModeShared),
              let cb = queue.makeCommandBuffer(), let enc = cb.makeComputeCommandEncoder() else {
            throw MetalError.bufferAlloc
        }
        enc.setComputePipelineState(try mulMVPipeline("kernel_mul_mv_q8_0_f32_pair", nsg: nsg))
        args.withUnsafeBytes { enc.setBytes($0.baseAddress!, length: args.count, index: 0) }
        enc.setBuffer(wa, offset: 0, index: 1); enc.setBuffer(wb, offset: 0, index: 2)
        enc.setBuffer(xb, offset: 0, index: 3)
        enc.setBuffer(oa, offset: 0, index: 4); enc.setBuffer(ob, offset: 0, index: 5)
        enc.setThreadgroupMemoryLength(32 * 2 * 4, index: 0)
        enc.dispatchThreadgroups(MTLSize(width: (outDim + nr0 - 1) / nr0, height: 1, depth: 1),
                                 threadsPerThreadgroup: MTLSize(width: 32, height: Int(nsg), depth: 1))
        enc.endEncoding(); cb.commit(); cb.waitUntilCompleted()
        let pa = oa.contents().bindMemory(to: Float.self, capacity: outDim)
        let pb = ob.contents().bindMemory(to: Float.self, capacity: outDim)
