...
extension GraphContext {
    ...
    func indexerTopKMask(scores: GPUTensor, mask: GPUTensor,
        ...
        // Thread 0 builds the heap; the remaining lanes initialize the mask and
        // stride over the selected rows.  The dispatch therefore does not need
        // one lane per top-k entry (Pro uses 1024), and remains portable to GPU
        // families whose per-threadgroup limit is lower than 1024.
        let nth = min(max(32, min(keep, 256)), pso.maxTotalThreadsPerThreadgroup)
        guard nth >= 32 else {
    ...
    private func hcReduce(curHc: GPUTensor, mixerFn: GPUTensor, scale: GPUTensor, base: GPUTensor,
        ...
        if d.fusedHC && d.nHC == 4 {
            ...
            try hcSplitSinkhorn(mix: s.mix, scale: scale, base: base, out: s.split, nRows: 1,
                                sinkhornIters: d.sinkhornIterations, eps: hcEps)
