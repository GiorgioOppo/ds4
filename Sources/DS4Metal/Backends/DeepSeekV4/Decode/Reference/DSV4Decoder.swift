...
/// Resident full-model reference used only by @testable synthetic graph tests.
/// It is deliberately internal: unlike `StreamingDecoder`, this historical
/// implementation does not own the complete compressed NSA state needed by a
/// production DeepSeek-V4 runtime.
final class DSV4Decoder {
    let rt: MetalRuntime
    let d: DSV4Dims
    let rope: RopeParams
    ...
    init(rt: MetalRuntime, dims: DSV4Dims, rope: RopeParams, layers: [LayerWeights],
    ...
    func forward(token: Int, pos: Int, nKeys: Int) throws -> [Float] {
        ...
        for i in 0..<layers.count {
            let layerRope = geometry?.ropeParams(layer: i) ?? DSV4Shape.ropeParams(layer: i)
            try ctx.decodeLayer(curHc: cur, w: layers[i], s: scratch, d: d, rope: layerRope,
    ...
    typealias Sampling = DSV4Sampling
    ...
    func generate(prompt: [Int], maxNew: Int,
                  sampling: Sampling = Sampling(), eos: Int? = nil) throws -> [Int] {
