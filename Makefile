...
help:
	...
	@echo "  make test-metal-qwen4-hc  Compare HC-down prefill tiles on Metal"
	@echo "  make check-metal-qwen4-hc  Compile HC-down tests without GPU dispatch"
...
.PHONY: test-metal-qwen4-hc check-metal-qwen4-hc
test-metal-qwen4-hc: tests/test_metal_qwen4_hc
	./tests/test_metal_qwen4_hc
...
tests/test_metal_qwen4_moe_down: tests/test_metal_qwen4_moe_down.m
	$(CC) $(filter-out -ffast-math,$(OBJCFLAGS)) -o $@ $< $(METAL_LDLIBS)

.PHONY: test-metal-qwen4-moe-down check-metal-qwen4-moe-down
test-metal-qwen4-moe-down: tests/test_metal_qwen4_moe_down
	./tests/test_metal_qwen4_moe_down
...
tests/test_qwen4_vision_bf16: tests/test_qwen4_vision_bf16.c ds4_metal.o ds4_image.o ds4_gpu.h
	$(CC) $(filter-out -ffast-math,$(CFLAGS)) -I. -o $@ $< ds4_metal.o ds4_image.o $(METAL_LDLIBS)
...
clean:
	...
	rm -f tests/test_bonsai_vision_model tests/test_qwen4_vision_bf16 tests/test_bonsai_rows
	rm -f tests/test_bonsai_pq2_prefill_load
...
tests/test_bonsai_vision_model: tests/test_bonsai_vision_model.c $(CORE_OBJS)
	$(CC) $(filter-out -ffast-math,$(CFLAGS)) -I. -o $@ $^ $(METAL_LDLIBS)

tests/test_bonsai_rows: tests/test_bonsai_rows.m ds4_bonsai.o ds4_bonsai_metal.o metal/bonsai.metal.inc
	$(CC) $(filter-out -ffast-math,$(OBJCFLAGS)) -I. -o $@ $< ds4_bonsai.o ds4_bonsai_metal.o $(METAL_LDLIBS)
...
tests/test_bonsai_prefill_pair: tests/test_bonsai_prefill_pair.m metal/bonsai.metal.inc
	$(CC) $(filter-out -ffast-math,$(OBJCFLAGS)) -o $@ $< $(METAL_LDLIBS)

tests/test_bonsai_attention_batch: tests/test_bonsai_attention_batch.m metal/bonsai.metal.inc
	$(CC) $(filter-out -ffast-math,$(OBJCFLAGS)) -o $@ $< $(METAL_LDLIBS)

tests/test_bonsai_bf16_pair_batch: tests/test_bonsai_bf16_pair_batch.m metal/bonsai.metal.inc
	$(CC) $(filter-out -ffast-math,$(OBJCFLAGS)) -o $@ $< $(METAL_LDLIBS)

tests/test_bonsai_pq2_prefill_load: tests/test_bonsai_pq2_prefill_load.m metal/bonsai.metal.inc
	$(CC) $(filter-out -ffast-math,$(OBJCFLAGS)) -o $@ $< $(METAL_LDLIBS)
...
test-bonsai-metal: tests/test_bonsai_fullrows tests/test_bonsai_metal tests/test_bonsai_graph_metal tests/test_bonsai_mm tests/test_bonsai_mma tests/test_bonsai_pq2_decode tests/test_bonsai_gdn_prefill tests/test_bonsai_pairs tests/test_bonsai_prefill_pair tests/test_bonsai_attention_batch tests/test_bonsai_bf16_pair_batch tests/test_bonsai_pq2_prefill_load tests/test_bonsai_rows tests/test_bonsai_ptq
	...
	MTL_DEBUG_LAYER=1 ./tests/test_bonsai_prefill_pair
	MTL_DEBUG_LAYER=1 ./tests/test_bonsai_attention_batch
