...
/// Validates the INT4 W4A16 GEMM kernel (`gemm_int4_w4a16_to_f32` /
/// `gemm_int4_w4a16_bf16_to_f32`) against a CPU reference. Same
/// structure as `Int8GemmTests` but with the [-8, 7] quantization range,
/// nibble-packed storage, and a looser tolerance to absorb the larger
/// quantization step (max-abs/14 vs max-abs/254).
final class Int4GemmTests: XCTestCase {

    func testInt4GEMMSquare() throws {
        let M = 2, K = 256, N = 256
        try runInt4Case(M: M, K: K, N: N, seedX: 11, seedW: 22, tolerance: 0.10)
    }

    func testInt4GEMMAsymmetric() throws {
        let M = 3, K = 384, N = 64
        try runInt4Case(M: M, K: K, N: N, seedX: 31, seedW: 41, tolerance: 0.12)
    }

    func testInt4GEMMMoEExpertShape() throws {
        let M = 1, K = 512, N = 128
        try runInt4Case(M: M, K: K, N: N, seedX: 51, seedW: 61, tolerance: 0.12)
    }

    func testInt4GEMMBF16Input() throws {
        let M = 2, K = 256, N = 128
        try runInt4CaseBF16(M: M, K: K, N: N, seedX: 71, seedW: 81, tolerance: 0.15)
    }

    // ---- helpers ----

    private func runInt4Case(M: Int, K: Int, N: Int,
        ...
        let (wPacked, wScaleF16, wDequant) = quantizeRefInt4(wArr, N: N, K: K)

        let x = xArr.withUnsafeBytes { Tensor.from(bytes: $0, shape: [M, K], dtype: .f32) }
        let w = wPacked.withUnsafeBytes {
            Tensor.from(bytes: $0, shape: [N, K], dtype: .i4)
        }
        let blocksK = K / 128
    ...
    private func runInt4CaseBF16(M: Int, K: Int, N: Int,
                                  seedX: UInt64, seedW: UInt64,
                                  tolerance: Float) throws {
        precondition(K % 128 == 0)
        let xArr = randomArray(M * K, seed: seedX, scale: 0.5)
    ...
    /// CPU reference INT4 quantizer matching `Int4Quant.quantizeRowFromFloatI4`.
    /// Returns (packed weight bytes, F16 group scales, dequantized FP32 matrix).
    private func quantizeRefInt4(_ w: [Float], N: Int, K: Int)
        -> (weight: [UInt8], scaleF16: [UInt16], dequant: [Float])
    {
        let blocksK = K / 128
        let packedRowBytes = K / 2
        ...
        for n in 0..<N {
            for kb in 0..<blocksK {
                ...
                let s: Float = maxAbs == 0 ? 0 : maxAbs / 7.0
                let invS: Float = maxAbs == 0 ? 0 : 7.0 / maxAbs
                scaleF16[n * blocksK + kb] = floatToF16(s)
                let sRoundedF16 = Self.f16ToFloat(floatToF16(s))
                for k in stride(from: 0, to: 128, by: 2) {
                    ...
                    let lo = UInt8(truncatingIfNeeded: c0) & 0x0F
                    let hi = UInt8(truncatingIfNeeded: c1) & 0x0F
                    weight[n * packedRowBytes + (k0 + k) / 2] = (hi << 4) | lo
