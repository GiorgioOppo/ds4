...
final class LoadStrategyTests: XCTestCase {
    ...
    /// (threshold is inclusive on the safe side).
    ...
    /// `largest_shard > available` is a hard error regardless of total.
    func testHardErrorWhenShardLargerThanAvailable() {
        XCTAssertThrowsError(
            try LoadPlan.decideForTesting(
                shards: [(u("big"), 20 * 1024 * 1024 * 1024)],
                availableRAM: 16 * 1024 * 1024 * 1024,
                override: nil)
        ) { error in
            guard case LoadStrategyError.shardTooLarge(let maxShard, let avail, let url) = error else {
                XCTFail("expected .shardTooLarge, got \(error)")
                return
            }
            XCTAssertEqual(maxShard, 20 * 1024 * 1024 * 1024)
            XCTAssertEqual(avail, 16 * 1024 * 1024 * 1024)
            XCTAssertEqual(url.lastPathComponent, "big.safetensors")
        }
    }

    /// Even `--load-strategy preload` doesn't bypass the hard error.
    func testHardErrorWinsOverPreloadOverride() {
        XCTAssertThrowsError(
            ...
            XCTAssertTrue(error is LoadStrategyError)
        }
    }

    /// Even `--load-strategy mmap` doesn't bypass the hard error.
