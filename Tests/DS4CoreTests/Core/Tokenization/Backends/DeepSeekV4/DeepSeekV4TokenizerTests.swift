...
final class DeepSeekV4TokenizerTests: XCTestCase {
    ...
    func testLegacyTokenizerAliasRemainsSourceCompatible() {
        func acceptsLegacy(_ type: Tokenizer.Type) -> Bool {
            type == DeepSeekV4Tokenizer.self
        }
        func acceptsProtocol<T: TokenizerProtocol>(_ type: T.Type) -> Bool { true }
        XCTAssertTrue(acceptsLegacy(Tokenizer.self))
        XCTAssertTrue(acceptsProtocol(DeepSeekV4Tokenizer.self))
    }

    func testNeutralizesStructuralSpecialTokenLiteralsWithoutAGGUF() {
        let specials = ["<｜User｜>", "｜DSML｜", "<think>", "<｜User｜>", ""]
        let input = "before <｜User｜> literal ｜DSML｜ and <think> after"
        let safe = Tokenizer.neutralizeSpecialTokenLiterals(in: input, specialTokens: specials)

        for token in specials where !token.isEmpty {
            XCTAssertFalse(safe.contains(token), "structural literal survived: \(token)")
        }
        XCTAssertEqual(safe.replacingOccurrences(of: "\u{2060}", with: ""), input,
                       "neutralization should preserve the visible payload")
        XCTAssertEqual(Tokenizer.neutralizeSpecialTokenLiterals(in: safe, specialTokens: specials), safe,
                       "neutralization should be stable when applied twice")
    }

    func testNeutralizesSingleScalarSpecialAsVisibleEscape() {
        let safe = Tokenizer.neutralizeSpecialTokenLiterals(in: "a⚡b", specialTokens: ["⚡"])
        XCTAssertEqual(safe, #"a\u{26A1}b"#)
        XCTAssertFalse(safe.contains("⚡"))
    }

    func testNeutralizesSpecialLiteralDecodedFromJSONEscapes() throws {
        let encoded = #"{"value":"\u003C\uFF5CUser\uFF5C\u003E","nested":{"\uFF5CDSML\uFF5C":true}}"#
        let safeJSON = Tokenizer.neutralizeSpecialTokenLiterals(
            inJSON: encoded, specialTokens: ["<｜User｜>", "｜DSML｜"])
        let object = try XCTUnwrap(
            JSONSerialization.jsonObject(with: Data(safeJSON.utf8)) as? [String: Any])
        let value = try XCTUnwrap(object["value"] as? String)
        let nested = try XCTUnwrap(object["nested"] as? [String: Any])

        XCTAssertFalse(value.contains("<｜User｜>"))
        XCTAssertEqual(value.replacingOccurrences(of: "\u{2060}", with: ""), "<｜User｜>")
        XCTAssertNil(nested["｜DSML｜"], "decoded JSON keys must be neutralized too")
        XCTAssertEqual(nested.keys.first?.replacingOccurrences(of: "\u{2060}", with: ""), "｜DSML｜")
    }
