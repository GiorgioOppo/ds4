...
final class DeepSeekV4ConfigurationTests: XCTestCase {
    ...
    func testLegacyTypeAliasesRemainSourceCompatible() {
        let legacyShape: ModelShape = .flash
        let explicitShape: DeepSeekV4Shape = legacyShape
        XCTAssertEqual(explicitShape, .flash)

        let legacyVariant: ModelVariant = .pro
        let explicitVariant: DeepSeekV4Variant = legacyVariant
        XCTAssertEqual(explicitVariant, .pro)

        XCTAssertEqual(ModelDefaults.rmsEps, DeepSeekV4Defaults.rmsEps)
        XCTAssertEqual(ModelDefaults.ropeOrigCtx, DeepSeekV4Defaults.ropeOrigCtx)
    }
    ...
    func testRealModelConfig() throws {
        ...
        let legacy = try ModelConfig(model: model)
        let cfg: DeepSeekV4Configuration = legacy
        ...
        XCTAssertEqual(cfg.descriptor.architecture.id, .deepSeekV4)
        XCTAssertEqual(cfg.descriptor.architecture.backendAvailability, .implemented)
        XCTAssertTrue(cfg.descriptor.capabilities.contains(.mixtureOfExperts))
    }
}
