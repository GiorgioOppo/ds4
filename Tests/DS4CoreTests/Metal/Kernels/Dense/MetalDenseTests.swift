...
final class MetalDenseTests: XCTestCase {
    ...
    func testQ8PairIsBitExactToSeparateMatvecs() throws {
        let rt: MetalRuntime
        do { rt = try MetalRuntime() }
        catch { throw XCTSkip("Metal unavailable: \(error)") }
        var seed: UInt64 = 0xC08A1
        func rnd() -> Float {
            seed = seed &* 6364136223846793005 &+ 1442695040888963407
        ...
        for (inDim, outDim) in [(512, 300), (4096, 512), (1024, 256)] {
            var a: [UInt8] = [], b: [UInt8] = []
            for _ in 0..<outDim {
                a.append(contentsOf: quantizeQ8_0((0..<inDim).map { _ in rnd() }))
                b.append(contentsOf: quantizeQ8_0((0..<inDim).map { _ in rnd() }))
            }
            let x = (0..<inDim).map { _ in rnd() }
            let separateA = try rt.matmulQ8_0(weight: a, activation: x, inDim: inDim, outDim: outDim)
            let separateB = try rt.matmulQ8_0(weight: b, activation: x, inDim: inDim, outDim: outDim)
            let paired = try rt.matmulQ8_0Pair(weightA: a, weightB: b, activation: x,
                                               inDim: inDim, outDim: outDim)
            XCTAssertEqual(paired.a, separateA, "paired Q8 output A must be bit-exact")
            XCTAssertEqual(paired.b, separateB, "paired Q8 output B must be bit-exact")
        }
    }
