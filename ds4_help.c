...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ssd-streaming-cache-experts N|NGB", "SSD streaming: N is an exact dynamic expert count (Metal SSD+DSpark can A/B an opt-in context-estimate cap with DS4_METAL_DSPARK_SAFE_EXPERT_COUNT=1); NGB is an accounted routed budget that also reserves two full prefill layers. Auto: 80% working set minus fixed weights.");
