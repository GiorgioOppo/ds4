...
bool ds4_session_vision_state_matches(const ds4_session *s,
                                      const ds4_vision_span *images,
                                      size_t image_count);
/* Position-independent image prefix: every historical image matches by row
 * count, grid geometry, layout and fingerprint, ignoring token positions
 * (which rebase repairs) and allowing appended request images. Lets the server
 * routing probe admit image-append continuations while still rejecting image
 * identity mismatches. */
 ...
 * (for example, matching tool-call IDs). Checks every fingerprint, row count,
 * grid geometry and layout; on failure, leaves spans unchanged. This does not
 * verify the text history. */
 ...
 * token_start/row-count/grid geometry/layout/fingerprint per span).
