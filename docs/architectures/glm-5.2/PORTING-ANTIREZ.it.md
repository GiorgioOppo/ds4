...
| Decoder | oracle CPU first-token E decode step (cablaggio esatto di `glm_graph_forward_token`: store cache prima di selezione/attenzione, fill-range/top-k con `visible = pos+1`, IndexShare verbatim), composizioni GPU di entrambi sui kernel validati confrontate con gli oracle; grafo residente della fase attenzione (pesi caricati una volta, cache GPU in place, un command buffer nel percorso fill-range) in parità con l'executor; motore reale GLM52ResidentModel (caricamento pesi dal GGUF, embedding row, prefill token-per-token, greedy decode; esperti IQ2_XXS supportati con kernel + dequant di riferimento) | 
