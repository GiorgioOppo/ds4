...
|---|---|---|
| **Model downloads** (`ModelDownloader` -> `huggingface.co`) | **HTTPS/TLS** through Foundation `URLSession`; **SHA-256** (`CryptoKit.SHA256`) to verify downloaded GGUF integrity | OS-provided TLS plus hashing; consistent with the current `NO` inventory. |
