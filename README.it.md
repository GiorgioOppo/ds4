...
**Novità: DeepSeek Vision sperimentale nella chat locale.** Scegli dal catalogo
il checkpoint Vision-Exp e il suo encoder separato, poi allega le immagini.
L'interfaccia aggiunge avvio guidato, ricerca delle chat e un composer più
accessibile. Configurazione, verifiche e limiti: [DeepSeek Vision](docs/DEEPSEEK-VISION.it.md).

**Mini Tool Bench** integra Terminal-Bench-Local (prima Terminal Bench Mini),
con Core-19 e la suite storica Mini-20. Un pulsante prepara Podman e avvia i test
usando il modello caricato, con log dal vivo e importazione degli esiti verificati:
[configurazione e risultati](Sources/DwarfStar/Features/MiniToolBench/README.md).
...
| `DS4_EXPERT_ASYNC_SPLIT` | `=0` disabilita; on | Fork/join esatto in stile C per i layer misti della slot-cache sulla forma Flash IQ2_XXS/Q2_K: gli esperti mancanti vengono caricati in background mentre Metal calcola una copia immutabile dei gate/up residenti; dopo il join, una sola coppia differita più il down-tail completa il layer. Una lease di lettura GPU esclude scritture look-ahead ritardate e il dispatch IQ2 indiretto usa l'`NSG=2` fisso e sicuro del C. Costa circa 25 MiB di scratch riutilizzabile; casi non supportati/tutti-hit/tutti-miss mantengono il percorso ordinario. L'A/B finale da 24 token ha misurato 3,77→4,06 tok/s (+7,7%) con 3.232.000/3.232.000 logit identici bit per bit; anche il run inverso da 12 token è passato esattamente e ha misurato +6,6%. `=0` è il rollback sincrono per gli A/B. |
