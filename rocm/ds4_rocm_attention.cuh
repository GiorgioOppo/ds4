...
/* Non-causal attention used by the DSpark draft block. Every draft query sees
 * the complete visible raw-KV ring plus the learned per-head sink. Keep the
 * scalar accumulation order aligned with the CUDA reference path. */
__global__ static void attention_noncausal_raw_batch_heads_kernel(
        float *heads,
        const float *sinks,
        const float *q,
